package com.wenguang.ai.assistant.lmstudio
...
class LMStudioClient {
    ...
    private val executorService = ThreadPoolExecutor(
        ...
        ThreadFactory { r ->
            Thread(r, "lmstudio-client-${System.currentTimeMillis()}-${r.hashCode()}")
        }
    )

    private var lmStudioApiUrl = "http://localhost:1234/v1"
    ...
    fun updateConfiguration(apiUrl: String, modelName: String) {
        lmStudioApiUrl = apiUrl.removeSuffix("/")
    ...
    fun sendChatMessage(
        ...
        executorService.submit {
            try {
                ...
                println("LM Studio chat request failed: $e")
    ...
    private fun sendChatMessageWithRetry(
        ...
        try {
            ...
            println(I18nManager.getMessage("log.lmstudio.request.exception", currentAttempt, maxRetries, e.message ?: "Unknown error"))
            ...
            if (currentAttempt < maxRetries && ApiException.isRetryable(e)) {
                ...
                println(I18nManager.getMessage("log.lmstudio.request.final_failed"))
                val finalErrorMessage = ApiException.createFromModelConfig(
                    ModelConfig(apiUrl = lmStudioApiUrl, modelName = currentChatModel),
                    exception = e
    ...
    private fun streamRequest(
        ...
        val url = URL("$lmStudioApiUrl/chat/completions")
        ...
        try {
            ...
            if (responseCode == HttpURLConnection.HTTP_OK) {
                ...
                // 解析并提供友好的错误信息
                val friendlyError = parseLMStudioError(errorMessage, responseCode)
    ...
    private fun processStreamLine(line: String, onChunk: (String) -> Unit) {
        if (line.startsWith("data: ")) {
            ...
            try {
                ...
                println(I18nManager.getMessage("log.lmstudio.stream.parse_failed", jsonData, e.message ?: "Unknown error"))
            }
        }
    }

    /**
     * 解析LM Studio错误并提供友好的提示信息
     */
    private fun parseLMStudioError(errorResponse: String, statusCode: Int): String {
        try {
            // 尝试解析JSON错误响应
            ...
            if (error != null) {
                ...
                return when {
                    errorCode == "model_not_found" || errorMessage?.contains("No models loaded") == true -> {
                        I18nManager.getMessage("api.error.lmstudio.no_model_loaded")
                    }

                    errorType == "invalid_request_error" -> {
                        when {
                            errorMessage?.contains("model") == true -> {
                                I18nManager.getMessage("api.error.lmstudio.invalid_model", errorMessage)
                            }

                            else -> {
                                I18nManager.getMessage(
                                    "api.error.lmstudio.invalid_request",
                                    errorMessage ?: "未知请求错误"
                                )
                            }
                        }
                    ...
                    statusCode == 500 -> {
                        I18nManager.getMessage("api.error.lmstudio.server_error")
                    }

                    statusCode == 404 -> {
                        I18nManager.getMessage("api.error.lmstudio.endpoint_not_found")
                    }

                    else -> {
                        I18nManager.getMessage("api.error.lmstudio.generic", errorMessage ?: "未知错误")
                    }
                }
            }
        } catch (e: Exception) {
            ...
            when {
                errorResponse.contains("No models loaded", ignoreCase = true) -> {
                    return I18nManager.getMessage("api.error.lmstudio.no_model_loaded")
                }

                errorResponse.contains("Connection refused", ignoreCase = true) -> {
                    return I18nManager.getMessage("api.error.lmstudio.connection_refused")
                }

                errorResponse.contains("timeout", ignoreCase = true) -> {
                    return I18nManager.getMessage("api.error.lmstudio.timeout")
                }

                statusCode == 404 -> {
                    return I18nManager.getMessage("api.error.lmstudio.endpoint_not_found")
                }
            }
        }

        // 默认错误信息
        return I18nManager.getMessage("api.error.lmstudio.generic", "HTTP $statusCode")
    }

    companion object {
        @Volatile
        private var INSTANCE: LMStudioClient? = null

        fun getInstance(): LMStudioClient {
            return INSTANCE ?: synchronized(this) {
                INSTANCE ?: LMStudioClient().also { INSTANCE = it }
            }
        }
    }
}